...
request_q.append(
    VllmCaptionRequest(
        ...
        inputs={
            ...
            "multi_modal_data": {"video": [(tensor, metadata)]}
        },
...
for request in finished:
    ...
    if request.stage2_prompt is None:
        results[original_idx] = _make_window_result(request, token_counts[original_idx])
...
def make_refined_llm_request(request, processor, refine_prompt):
    ...
    # Reuses original video payload
    video_data = request.inputs["multi_modal_data"]["video"]
    # Creates new message with refinement prompt
    message = make_message(final_prompt)
    inputs = make_prompt(message, video_data, processor)
...
**Step 8**: Return final results when all requests complete
```python
# File: vllm_interface.py
# while loop exits when len(results) == total_requests (original count, not including stage2)
return [results[i] for i in range(total_requests)]
# Returns: [VllmWindowResult(text="A person walking along a tree-lined path in autumn", ...)]
```
...
class VllmCaptionRequest:
    ...
    finish_reason: str | None = None  # Terminal vLLM finish reason
```
...
### Scenario 1: Caption results normalize to errors

**Symptoms:**
```python
results = vllm_caption(...)
# Result text is not usable or downstream window metadata shows caption_status="error"
```

**Possible Cause:** Caption generation or plugin decoding is producing no usable
text, so the caption stage normalizes the result to an error outcome.
...
```python
config = VllmConfig(model_variant="qwen", stage2_caption=True)
results = vllm_caption(..., stage2_prompts=["Refine..."])
# But result text isn't refined
```
...
**Symptoms:**
```python
# Submitted 100 requests, only got 95 results back
```
...
4. Check loop termination condition:
   ...
   if len(results) >= total_requests:
       print(f"Loop terminating: {len(results)} >= {total_requests}")
   ```

**Common causes:**
- Exception in `decode()` → no finished result until the exception is fixed
...
# In _caption_inflight_batching()
logger.debug(f"Request queue: {len(request_q)}, In-flight: {len(in_flight_requests)}, Results: {len(results)}")
