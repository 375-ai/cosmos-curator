...
class VideoSplitSplitConfig(BaseModel):
    ...
    limit_clips: int | None = Field(default=None, ge=1)
...
_PRESETS: dict[str, dict[str, dict[str, Any]]] = {
    ...
    "caption": {
        "balanced": {"enabled": True, "model": "qwen", "batch_size": 32},
        "off": {"enabled": False},
    },
    "execution": {
...
_VIDEO_SPLIT_TEMPLATE_DESCRIPTION = "Smallest runnable video_split config; relies on packaged defaults."

_VIDEO_SPLIT_TEMPLATE: dict[str, Any] = {
    "schema_version": 1,
    "kind": "video_split",
...
def video_split_config_template() -> dict[str, Any]:
    """Return a packaged user-authored ``video_split`` config template."""
    return copy.deepcopy(_VIDEO_SPLIT_TEMPLATE)
...
def video_split_template_payload() -> dict[str, Any]:
    """Return machine-readable template metadata for agent and tool use."""
    return {
        "kind": "video_split",
        "description": _VIDEO_SPLIT_TEMPLATE_DESCRIPTION,
        "required_fields": video_split_required_fields(),
        "config": video_split_config_template(),
    }
